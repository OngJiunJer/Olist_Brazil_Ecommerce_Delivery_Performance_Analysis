...
-- Create a base CTE to filter delivered orders
-- and calculate delivery duration (in days) from purchase to customer delivery
WITH base_orders AS (
    SELECT
        o.order_id,
        o.customer_id,
...
-- Enrich order data with customer location information
-- by joining the base orders with the customers table
orders_with_location AS (
    SELECT
        b.order_id,
...
year_month_level_aggregation AS (
	-- Aggregate delivery performance by yaer and month
	-- to analyze total orders and average delivery time per year and time
	SELECT
		deliver_year,
		deliver_month,
		COUNT(order_id) AS total_orders,
		AVG(CAST(CASE WHEN delivery_days < p99_delivery_days THEN delivery_days END AS FLOAT)) AS avg_delivery_days
	FROM orders_with_location_percentile
	GROUP BY deliver_year, deliver_month
	HAVING COUNT(order_id) > 100
)
SELECT *
FROM year_month_level_aggregation
ORDER BY avg_delivery_days DESC;
